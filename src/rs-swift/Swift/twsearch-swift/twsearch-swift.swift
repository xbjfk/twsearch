public func random_scramble_for_event(_ event: Event) throws -> Alg {
    try { let val = __swift_bridge__$random_scramble_for_event({event.isOwned = false; return event.ptr;}()); if val.is_ok { return Alg(ptr: val.ok_or_err!) } else { throw PuzzleError(ptr: val.ok_or_err!) } }()
}
